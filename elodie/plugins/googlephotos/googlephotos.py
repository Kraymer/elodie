...
class GooglePhotos(PluginBase):
    ...
    def after(self, file_path, destination_folder, final_file_path, metadata):
        ...
        if extension in Photo.extensions or extension in Video.extensions:
            ...
            self.log(
                u"Skipping {} which is not a supported media type.".format(
                    final_file_path
                )
            )
    ...
    def upload(self, path_to_photo):
        ...
        if upload_token.status_code != 200 or not upload_token.content:
            self.log(
                "Uploading media failed: ({}) {}".format(
                    upload_token.status_code, upload_token.content
                )
            )
            return None
        ...
        if (
            "newMediaItemResults" not in resp
            or "status" not in resp["newMediaItemResults"][0]
            or "message" not in resp["newMediaItemResults"][0]["status"]
            or (
                resp["newMediaItemResults"][0]["status"]["message"] != "Success"
                and resp["newMediaItemResults"][0]["status"]["message"]  # photos
                != "OK"  # videos
            )
        ):
