...
def test_application_directory_default():
    ...
    assert (
        constants.application_directory == expected_path
...
def test_hash_db():
    assert constants.hash_db == "{}/hash.json".format(
        constants.application_directory
    ), constants.hash_db


def test_location_db():
    assert constants.location_db == "{}/location.json".format(
        constants.application_directory
    ), constants.location_db
...
def test_exiftool_config():
    path = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
    assert (
        "{}/configs/ExifTool_config".format(path) == constants.exiftool_config
    ), constants.exiftool_config


def test_mapquest_base_url_default():
    assert (
        constants.mapquest_base_url == "https://open.mapquestapi.com"
