...
sys.path.insert(
    0, os.path.abspath(os.path.dirname(os.path.dirname(os.path.realpath(__file__))))
)
...
@mock.patch(
    "elodie.config.config_file", "%s/config.ini-googlephotos-set-session" % gettempdir()
)
...
@mock.patch(
    ...
    "%s/config.ini-googlephotos-after-supported" % gettempdir(),
)
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-googlephotos-after-unsupported" % gettempdir(),
)
...
@mock.patch(
    "elodie.config.config_file", "%s/config.ini-googlephotos-upload" % gettempdir()
)
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-googlephotos-upload-session-fail" % gettempdir(),
)
def test_googlephotos_upload_session_fail():
    with open(
        "%s/config.ini-googlephotos-upload-session-fail" % gettempdir(), "w"
    ) as f:
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-googlephotos-upload-invalid-empty" % gettempdir(),
)
def test_googlephotos_upload_invalid_empty():
    with open(
        "%s/config.ini-googlephotos-upload-invalid-empty" % gettempdir(), "w"
    ) as f:
        f.write(config_string_fmt)
    if hasattr(load_config, "config"):
        del load_config.config
...
@mock.patch(
    "elodie.config.config_file", "%s/config.ini-googlephotos-upload-dne" % gettempdir()
)
...
@mock.patch(
    "elodie.config.config_file", "%s/config.ini-googlephotos-batch" % gettempdir()
)
