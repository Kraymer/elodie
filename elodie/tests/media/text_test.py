...
def test_get_coordinate_latitude():
    text = Text(helper.get_file("valid.txt"))
    text.get_metadata()
    assert text.get_coordinate("latitude") == "51.521435", text.get_coordinate(
        "latitude"
    )
...
def test_get_coordinate_longitude():
    text = Text(helper.get_file("valid.txt"))
    text.get_metadata()
    assert text.get_coordinate("longitude") == "0.162714", text.get_coordinate(
        "longitude"
    )
...
def test_get_date_taken():
    ...
    assert date_taken == helper.time_convert(
        (2016, 4, 7, 11, 15, 26, 3, 98, 0)
    ), date_taken
...
def test_get_date_taken_from_invalid():
    ...
    seconds_since_epoch = min(os.path.getmtime(origin), os.path.getctime(origin))
...
def test_set_date_taken():
    ...
    assert (
        helper.time_convert((2013, 9, 30, 7, 6, 5, 0, 273, 0)) != metadata["date_taken"]
    ), metadata["date_taken"]
    ...
    assert (
        helper.time_convert((2013, 9, 30, 7, 6, 5, 0, 273, 0))
        == metadata_new["date_taken"]
    ), metadata_new["date_taken"]
...
def test_set_location():
    ...
    assert not helper.isclose(
        origin_metadata["latitude"], 11.1111111111
    ), origin_metadata["latitude"]
    assert not helper.isclose(
        origin_metadata["longitude"], 99.9999999999
    ), origin_metadata["longitude"]
...
def test_set_original_name():
    ...
    assert metadata_updated["original_name"] == random_file_name, metadata_updated[
        "original_name"
    ]
