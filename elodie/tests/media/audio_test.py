...
def test_set_location_minus():
    ...
    # Verify that original audio has different location info that what we
    #   will be setting and checking
    assert not helper.isclose(origin_metadata["latitude"], 11.111111), origin_metadata[
        "latitude"
    ]
    assert not helper.isclose(origin_metadata["longitude"], 99.999999), origin_metadata[
        "longitude"
    ]
