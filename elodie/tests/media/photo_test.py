...
def test_get_date_taken_without_exif():
    ...
    date_taken_from_file = time.gmtime(
        min(os.path.getmtime(source), os.path.getctime(source))
    )
...
def _test_photo_type_get(type, date):
    ...
    assert metadata["date_taken"] == helper.time_convert(date), "{} date {}".format(
        type, metadata["date_taken"]
    )
...
def _test_photo_type_set(type, date):
    ...
    assert helper.isclose(metadata["latitude"], 11.1111111111), "{} lat {}".format(
        type, metadata["latitude"]
    )
    assert helper.isclose(metadata["longitude"], 99.9999999999), "{} lon {}".format(
        type, metadata["latitude"]
    )
