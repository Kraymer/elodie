...
prefer_english_names=False
    ...
    config = load_config()
    assert config["MapQuest"]["key"] == "your-api-key-goes-here", config.get(
        "MapQuest", "key"
    )
...
@patch(
    ...
    "%s/config.ini-load-plugin-config-unset-backwards-compat" % gettempdir(),
)
def test_load_plugin_config_unset_backwards_compat():
    with open(
        "%s/config.ini-load-plugin-config-unset-backwards-compat" % gettempdir(), "w"
    ) as f:
        f.write(
            """
        """
...
@patch(
    "elodie.config.config_file",
    "%s/config.ini-load-plugin-config-exists-not-set" % gettempdir(),
)
def test_load_plugin_config_exists_not_set():
    with open(
        "%s/config.ini-load-plugin-config-exists-not-set" % gettempdir(), "w"
    ) as f:
        f.write(
            """
[Plugins]
...
@patch(
    "elodie.config.config_file", "%s/config.ini-load-plugin-config-one" % gettempdir()
)
...
@patch(
    "elodie.config.config_file",
    "%s/config.ini-load-plugin-config-one-with-invalid" % gettempdir(),
)
...
@patch(
    "elodie.config.config_file", "%s/config.ini-load-plugin-config-many" % gettempdir()
)
