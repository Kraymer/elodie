...
def test_load_plugins_unset_backwards_compat():
    with open(
        "%s/config.ini-load-plugins-unset-backwards-compat" % gettempdir(), "w"
    ) as f:
        f.write(
            """
        """
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-load-plugins-exists-not-set" % gettempdir(),
)
...
@mock.patch(
    "elodie.config.config_file", "%s/config.ini-load-plugins-one" % gettempdir()
)
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-load-plugins-one-with-invalid" % gettempdir(),
)
...
@mock.patch(
    "elodie.config.config_file", "%s/config.ini-load-plugins-many" % gettempdir()
)
...
plugins=ThrowError,Dummy
    ...
    assert plugins.classes["ThrowError"].__name__ == "ThrowError", plugins.classes[
        "ThrowError"
    ].__name__
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-load-plugins-many-with-invalid" % gettempdir(),
)
...
@mock.patch(
    "elodie.config.config_file", "%s/config.ini-throw-error-one-of-many" % gettempdir()
)
...
@mock.patch(
    "elodie.config.config_file", "%s/config.ini-throw-runtime-error" % gettempdir()
)
