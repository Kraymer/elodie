...
def test_create_directory_recursive_success():
    filesystem = FileSystem()
    folder = os.path.join(
        helper.temp_dir(), helper.random_string(10), helper.random_string(10)
    )
    status = filesystem.create_directory(folder)
...
def test_get_file_name_with_original_name_title_exif():
    ...
    assert file_name == helper.path_tz_fix(
        "2015-12-05_00-59-26-foobar-foobar-title.jpg"
    ), file_name
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-filename-custom-with-title" % gettempdir(),
)
...
name=%date-%original_name-%title.%extension
    ...
    assert file_name == helper.path_tz_fix(
        "2015-12-05-with-title-some-title.jpg"
    ), file_name


@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-filename-custom-with-empty-value" % gettempdir(),
)
def test_get_file_name_custom_with_empty_value():
    with open(
        "%s/config.ini-filename-custom-with-empty-value" % gettempdir(), "w"
    ) as f:
        f.write(
            """
[File]
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-filename-custom-with-lowercase" % gettempdir(),
)
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-filename-custom-with-invalidcase" % gettempdir(),
)
def test_get_file_name_custom_with_invalid_capitalization():
    with open(
        "%s/config.ini-filename-custom-with-invalidcase" % gettempdir(), "w"
    ) as f:
        f.write(
            """
[File]
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-filename-custom-with-uppercase" % gettempdir(),
)
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-original-with-camera-make-and-model" % gettempdir(),
)
def test_get_folder_path_with_camera_make_and_model():
    with open(
        "%s/config.ini-original-with-camera-make-and-model" % gettempdir(), "w"
    ) as f:
        f.write(
            """
[Directory]
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-original-with-camera-make-and-model-fallback" % gettempdir(),
)
def test_get_folder_path_with_camera_make_and_model_fallback():
    with open(
        "%s/config.ini-original-with-camera-make-and-model-fallback" % gettempdir(), "w"
    ) as f:
        f.write(
            """
[Directory]
...
@mock.patch(
    "elodie.config.config_file", "%s/config.ini-int-in-component-path" % gettempdir()
)
...
@mock.patch(
    "elodie.config.config_file", "%s/config.ini-combined-date-and-album" % gettempdir()
)
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-combined-date-album-location-fallback" % gettempdir(),
)
def test_get_folder_path_with_album_and_location_fallback():
    # gh-279
    with open(
        "%s/config.ini-combined-date-album-location-fallback" % gettempdir(), "w"
    ) as f:
        f.write(
            """
[Directory]
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-original-default-unknown-location" % gettempdir(),
)
def test_get_folder_path_with_original_default_unknown_location():
    with open(
        "%s/config.ini-original-default-with-unknown-location" % gettempdir(), "w"
    ) as f:
        f.write("")
    if hasattr(load_config, "config"):
        del load_config.config
...
full_path=%date/%location
    ...
    assert path == os.path.join(
        "2015-12-05", "United States of America-California-Sunnyvale"
    ), path
...
def test_process_file_invalid():
    ...
    destination = filesystem.process_file(
        origin, temporary_folder, media, allowDuplicate=True
    )
...
def test_process_file_plain():
    ...
    assert (
        helper.path_tz_fix(
            os.path.join(
                "2015-12-Dec", "Unknown Location", "2015-12-05_00-59-26-photo.jpg"
            )
        )
        in destination
    ), destination
...
def test_process_file_with_location():
    ...
    assert (
        helper.path_tz_fix(
            os.path.join("2015-12-Dec", "Sunnyvale", "2015-12-05_00-59-26-photo.jpg")
        )
        in destination
    ), destination
...
def test_process_file_with_location_and_title():
    ...
    assert (
        helper.path_tz_fix(
            os.path.join(
                "2015-12-Dec", "Sunnyvale", "2015-12-05_00-59-26-photo-some-title.jpg"
            )
        )
        in destination
    ), destination
...
def test_process_file_with_album():
    ...
    assert (
        helper.path_tz_fix(
            os.path.join("2015-12-Dec", "Test Album", "2015-12-05_00-59-26-photo.jpg")
        )
        in destination
    ), destination
...
def test_process_file_with_album_and_title():
    ...
    assert (
        helper.path_tz_fix(
            os.path.join(
                "2015-12-Dec", "Test Album", "2015-12-05_00-59-26-photo-some-title.jpg"
            )
        )
        in destination
    ), destination
...
def test_process_video_with_album_then_title():
    ...
    assert (
        helper.path_tz_fix(
            os.path.join(
                "2015-01-Jan", "test_album", "2015-01-19_12-45-11-movie-test_title.mov"
            )
        )
        in destination
    ), destination
...
full_path=%date/%album|"fallback"
    ...
    assert (
        helper.path_tz_fix(
            os.path.join("2015-12", "fallback", "2015-12-05_00-59-26-plain.jpg")
        )
        in destination
    ), destination
...
@mock.patch(
    "elodie.config.config_file", "%s/config.ini-multiple-directories" % gettempdir()
)
...
full_path=%year/%month/%day
    ...
    assert (
        helper.path_tz_fix(
            os.path.join("2015", "12", "05", "2015-12-05_00-59-26-plain.jpg")
        )
        in destination
    ), destination
    ...
    destination_second = filesystem.process_file(
        destination, temporary_folder, media_second, allowDuplicate=True
    )
    ...
    assert (
        destination.replace(".jpg", "-foo.jpg") == destination_second
    ), destination_second
...
def test_process_existing_file_without_changes():
    ...
    assert (
        helper.path_tz_fix(
            os.path.join(
                "2015-12-Dec", "Unknown Location", "2015-12-05_00-59-26-plain.jpg"
            )
        )
        in destination
    ), destination
...
@mock.patch(
    "elodie.config.config_file", "%s/config.ini-plugin-throw-error" % gettempdir()
)
...
@mock.patch(
    "elodie.config.config_file", "%s/config.ini-plugin-runtime-error" % gettempdir()
)
...
plugins=RuntimeError
    ...
    assert (
        "2015-12-Dec/Unknown Location/2015-12-05_00-59-26-plain.jpg" in destination
    ), destination
...
def test_set_utime_with_exif_date():
    ...
    filesystem.set_utime_from_metadata(
        media_initial.get_metadata(), media_initial.get_file_path()
    )
...
def test_set_utime_without_exif_date():
    ...
    assert final_stat.st_mtime == time.mktime(metadata_final["date_taken"]), (
        final_stat.st_mtime,
        time.mktime(metadata_final["date_taken"]),
    )
    assert initial_checksum == final_checksum
...
def test_should_not_exclude_with_multiple_with_non_matching_regex():
    filesystem = FileSystem()
    result = filesystem.should_exclude(
        "/some/path", {re.compile("foobar"), re.compile("dne")}
    )
    assert result == False, result
...
def test_should_exclude_with_multiple_with_one_matching_regex():
    filesystem = FileSystem()
    result = filesystem.should_exclude(
        "/some/path", {re.compile("foobar"), re.compile("some")}
    )
    assert result == True, result
...
def test_should_exclude_with_complex_matching_regex():
    filesystem = FileSystem()
    result = filesystem.should_exclude(
        "/var/folders/j9/h192v5v95gd_fhpv63qzyd1400d9ct/T/T497XPQH2R/UATR2GZZTX/2016-04-Apr/London/2016-04-07_11-15-26-valid-sample-title.txt",
        {re.compile("London.*\.txt$")},
    )
    assert result == True, result
...
def test_get_folder_path_definition_default():
    ...
    assert path_definition == [
        [("date", "%Y-%m-%b")],
        [("album", ""), ("location", "%city"), ('"Unknown Location"', "")],
    ], path_definition
...
full_path=%location/%date
    ...
    filesystem = FileSystem()
    path_definition = filesystem.get_folder_path_definition()
    expected = [[("location", "%country")], [("date", "%Y-%m-%d")]]
    if hasattr(load_config, "config"):
        del load_config.config
...
@mock.patch(
    "elodie.config.config_file", "%s/config.ini-multi-level-custom" % gettempdir()
)
