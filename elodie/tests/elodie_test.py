...
elodie = load_source(
    "elodie",
    os.path.abspath(
        "{}/../../elodie.py".format(os.path.dirname(os.path.realpath(__file__)))
    ),
)
...
def test_import_file_text():
    ...
    assert (
        helper.path_tz_fix(
            os.path.join(
                "2016-04-Apr", "London", "2016-04-07_11-15-26-valid-sample-title.txt"
            )
        )
        in dest_path
    ), dest_path
...
def test_import_file_audio():
    ...
    assert (
        helper.path_tz_fix(
            os.path.join("2016-01-Jan", "Houston", "2016-01-04_05-28-15-audio.m4a")
        )
        in dest_path
    ), dest_path
...
def test_import_file_video():
    ...
    assert (
        helper.path_tz_fix(
            os.path.join("2015-01-Jan", "California", "2015-01-19_12-45-11-video.mov")
        )
        in dest_path
    ), dest_path
...
@mock.patch(
    ...
    "%s/config.ini-import-file-with-single-config-exclude" % gettempdir(),
)
def test_import_file_with_single_config_exclude():
    ...
    name1=valid
            """
    with open(
        "%s/config.ini-import-file-with-single-config-exclude" % gettempdir(), "w"
    ) as f:
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-import-file-with-multiple-config-exclude" % gettempdir(),
)
def test_import_file_with_multiple_config_exclude():
    ...
    name2=valid
            """
    with open(
        "%s/config.ini-import-file-with-multiple-config-exclude" % gettempdir(), "w"
    ) as f:
        f.write(config_string)
...
def test_update_location_on_audio():
    ...
    assert metadata["latitude"] != metadata_processed["latitude"], metadata_processed[
        "latitude"
    ]
    assert helper.isclose(metadata_processed["latitude"], 37.36883), metadata_processed[
        "latitude"
    ]
    assert helper.isclose(
        metadata_processed["longitude"], -122.03635
    ), metadata_processed["longitude"]
...
def test_update_with_directory_passed_in():
    ...
    runner = CliRunner()
    result = runner.invoke(
        elodie._import, ["--destination", folder_destination, folder]
    )
    runner2 = CliRunner()
    ...
    updated_file_path = (
        "{}/2016-04-Apr/test/2016-04-07_11-15-26-valid-sample-title.txt".format(
            folder_destination
        )
    )
...
def test_update_invalid_file_exit_code():
    ...
    runner = CliRunner()
    result = runner.invoke(
        elodie._update, ["--album", "test", origin_invalid, origin_valid]
    )
    helper.restore_dbs()
...
def test_regenerate_valid_source():
    ...
    assert (
        "3c19a5d751cf19e093b7447297731124d9cc987d3f91a9d1872c3b1c1b15639a" in db.hash_db
    ), db.hash_db
...
@mock.patch(
    "elodie.config.config_file",
    "%s/config.ini-cli-batch-plugin-googlephotos" % gettempdir(),
)
def test_cli_batch_plugin_googlephotos():
    ...
    assert (
        'elodie/elodie/tests/files/plain.jpg uploaded successfully."}\n'
        in result.output
    ), result.output
    assert (
        'elodie/elodie/tests/files/no-exif.jpg uploaded successfully."}\n'
        in result.output
    ), result.output
...
def test_cli_debug_import():
    ...
    # import
    result = runner.invoke(
        elodie._import, ["--destination", "/does/not/exist", "/does/not/exist"]
    )
    assert "Could not find /does/not/exist\n" not in result.output, result.output
    result = runner.invoke(
        elodie._import,
        ["--destination", "/does/not/exist", "--debug", "/does/not/exist"],
    )
...
def test_cli_debug_update():
    ...
    assert "Could not find /does/not/exist\n" not in result.output, result.output
    result = runner.invoke(
        elodie._update, ["--location", "foobar", "--debug", "/does/not/exist"]
    )
    assert "Could not find /does/not/exist\n" in result.output, result.output
