...
def create_working_folder(format=None):
    ...
    folder = os.path.join(
        temporary_folder, random_string(10, format), random_string(10, format)
    )
    os.makedirs(folder)
...
def download_file(name, destination):
    try:
        ...
        if constants.python_version < 3:
            final_name = "{}/{}{}".format(
                destination, random_string(10), os.path.splitext(name)[1]
            )
            urllib.urlretrieve(url_to_file, final_name)
        else:
...
def path_tz_fix(file_name):
    if is_windows():
        ...
        tz_shift = old_div(
            (datetime.fromtimestamp(0) - datetime.utcfromtimestamp(0)).seconds, 3600
        )
        # replace timestamp in file_name
        m = re.search("(\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2})", file_name)
        t_date = datetime.fromtimestamp(
            time.mktime(time.strptime(m.group(0), "%Y-%m-%d_%H-%M-%S"))
        )
...
def isclose(a, b, rel_tol=1e-8):
    ...
    diff = abs(a - b)
    return diff <= abs(rel_tol * a) and diff <= abs(rel_tol * b)
...
def setup_module():
    exiftool_addedargs = ["-config", '"{}"'.format(constants.exiftool_config)]
