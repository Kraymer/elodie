...
def import_file(_file, destination, album_from_folder, trash, allow_duplicates):
    ...
    if not os.path.exists(_file):
        ...
        log.all('{"source":"%s", "error_msg":"Could not find %s"}' % (_file, _file))
        return
    ...
    dest_path = FILESYSTEM.process_file(
        _file, destination, media, allowDuplicate=allow_duplicates, move=False
    )
    if dest_path:
...
@click.option(
    "--file", type=click.Path(dir_okay=False), help="Import this file, if specified."
)
@click.option(
    "--album-from-folder",
    default=False,
...
def _import(
    ...
    for current_file in files:
        dest_path = import_file(
            current_file, destination, album_from_folder, trash, allow_duplicates
        )
...
@click.option(
    ...
    type=click.Path(file_okay=False),
    required=True,
    help="Source of your photo library.",
)
@click.option(
    "--debug",
    default=False,
...
def update_location(media, file_path, location_name):
    ...
    if (
        location_coords
        and "latitude" in location_coords
        and "longitude" in location_coords
    ):
        location_status = media.set_location(
            location_coords["latitude"], location_coords["longitude"]
        )
...
@click.option(
    "--location",
    help=(
        "Update the image location. Location "
...
@click.option(
    "--time",
    help=(
        "Update the image time. Time should be in "
...
def _update(album, location, time, title, paths, debug):
    ...
    for current_file in files:
        ...
        if updated:
            updated_media = Media.get_class_by_file(current_file, get_all_subclasses())
            ...
            dest_path = FILESYSTEM.process_file(
                current_file, destination, updated_media, move=True, allowDuplicate=True
            )
            log.info(u"%s -> %s" % (current_file, dest_path))
            log.all('{"source":"%s", "destination":"%s"}' % (current_file, dest_path))
